...
dois <- list.files('../apa_articles')  # nb remove "../" !!!

# Extract the results from each paper
for(doi in dois) {
  if(!file.exists(sprintf('../apa_articles/%s/results.csv', doi))) {  # nb remove "../" !!!
    
    filename <- sprintf('../apa_articles/%s/fulltext.txt', doi)  # nb remove "../" !!!
    ...
    if(!is.null(comparison)) {
      ...
      # write to disk
      write.csv(df, 
                sprintf('../apa_articles/%s/results.csv', doi),  # nb remove "../" !!!
                row.names = FALSE)
    }
    cat(sprintf('%s\n', doi)) 
  }
